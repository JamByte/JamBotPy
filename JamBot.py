...
async def on_message(message):
    ...
    if int(message.author.id) in jsondata["userid"][serverindex]:
        ...
        messagetext= "New cookie jar created, you have 10 cookies in it to start you off <@"+str(message.author.id)+">"
        embed()
        await client.send_message(message.channel, embed=cookieembed)
        idindex=jsondata["userid"][serverindex].index(int(message.author.id))
    ...
    if int(message.channel.id) in jsondata["channel"]:
        if int(message.author.id) in jsondata["userid"]:
                if int(message.author.id) in jsondata["usertime"]:
                    ...
                    if time.time()-int(jsondata["cooldown"][timeindex])>=15:
        ...
        if message.content.startswith('[bakecookie]') or message.content.startswith('[bakecookies]'):
            ...
            if jsondata["cookies"][serverindex][idindex]< gambleamount:
                ...
                if dice== 12 :
                    gamblenum = int(gambleamount*4)
                    jsondata["cookies"][serverindex][idindex]+=gamblenum
                    messagetext= '<@'+str(message.author.id)+'> You roll the dice and get a '+str(dice)+', you gained '+str(gamblenum)+' cookies, Jar count at '+str(jsondata["cookies"][serverindex][idindex])+' Cookies'
                elif dice== 11:
                    gamblenum = int(gambleamount*3)
                    jsondata["cookies"][serverindex][idindex]+=gamblenum
                    messagetext= '<@'+str(message.author.id)+'> You roll the dice and get a '+str(dice)+', you gained '+str(gamblenum)+' cookies, Jar count at '+str(jsondata["cookies"][serverindex][idindex])+' Cookies'
                    
                elif dice==10 or dice==9:
                    gamblenum = gambleamount
                    jsondata["cookies"][serverindex][idindex]+= gamblenum
                    messagetext='<@'+str(message.author.id)+'> You roll the dice and get a '+str(dice)+', you gained '+str(gamblenum)+' cookies, Jar count at '+str(jsondata["cookies"][serverindex][idindex])+' Cookies'
                elif dice==2:
                    jsondata["cookies"][serverindex][idindex] = int(jsondata["cookies"][serverindex][idindex]/4)
                    messagetext='<@'+str(message.author.id)+'> You roll the dice and get snake eyes, you lost 75% your cookies, Jar count at '+str(jsondata["cookies"][serverindex][idindex])+' Cookies'
                else:
...
'''
            ...
            if int(message.author.id) in [250138264768479232, 308209615911387139]:
            ...
            if int(message.author.id) in [250138264768479232, 308209615911387139]:
                    ...
                    if "-" in message.content:
                        ...
                        jsondata["cookies"][serverindex][idindex]+=(idcookie[1])
                        messagetext= "You have edited <@"+str(idcookie[0])+"> cookies to -"+str(idcookie[1])+" cookies"
                        embed()
                        await client.send_message(message.channel, embed=cookieembed)
                    else:
...
Current mix cost '''+str(jsondata["mixcost"][serverindex][idindex]*78)+''' Cookies
So Far you have collected '''+str(jsondata["mixtype"][serverindex][idindex])
            embed()
            await client.send_message(message.channel, embed=cookieembed)
        elif message.content.startswith('[buy mix]') :
            if len(mix)==len(jsondata["mixtype"][serverindex][idindex])+1:
                messagetext= "<@"+str(message.author.id)+"> You Own all the types already"
                embed()
                await client.send_message(message.channel, embed=cookieembed)
            if jsondata["cookies"][serverindex][idindex]<(int(78*jsondata["mixcost"][serverindex][idindex])):
                ...
                while mixtype2==0:
                    if mixtype in jsondata["mixtype"][serverindex][idindex]:
                         ...
                         mixtype2=1
                jsondata["cookies"][serverindex][idindex]-=int((int(78*jsondata["mixcost"][serverindex][idindex])))         
                jsondata["mixtype"][serverindex][idindex].append(mixtype)
                jsondata["mixcost"][serverindex][idindex]=int(jsondata["mixcost"][serverindex][idindex]*2)
...
[buy anticookie]'''
            ...
            if jsondata["cookies"][serverindex][idindex]<int(25936680*jsondata["demigodprice"][serverindex][idindex]):
                messagetext= "<@"+str(message.author.id)+"> You dont have any cookies in your jar use **[bakecookies]** to get more"
                embed()
                await client.send_message(message.channel, embed=cookieembed)
            else:
...
'''
            ...
            if int(message.author.id) in jsondata["usertime"]:
                timeindex = jsondata["usertime"].index(int(message.author.id))
                if (time.time()-int(jsondata["cooldown"][timeindex]))>=3:
                    jsondata["cooldown"][timeindex]=time.time()
                    command = 1
            else:
                jsondata["cooldown"].append(int(time.time()))
            ...
            if command ==1:
                ...
                messageevent=0
                ranevent = random.randint(150,464)
